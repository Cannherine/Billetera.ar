...
/**
 * Interfaz principal para el sistema de Billetera Virtual. Define las
 * operaciones disponibles para usuarios, cuentas e inversiones.
 */
public interface IBilletera {
	/**
	 * [Nuevo] 11) Registra una nueva empresa en el sistema. Lanza error si la
	 * empresa ya esta registrada o algun campo es inválido.
	 *
	...
	/**
	 * [Nuevo] 12) Agrega una persona autorizada a operar en nombre de una empresa.
	...
	/**
	 * 1) Registra un nuevo usuario en la plataforma. Lanza error si el usuario ya
	 * está registrado o algun campo es inválido.
	 *
	...
	/**
	 * 2) Crea una nueva cuenta de tipo regular para un usuario existente. Lanza
	 * error si el usuario no existe o el alias ya está registrado.
	 *
	...
	/**
	 * 2) Crea una nueva cuenta de tipo premium para un usuario existente. Lanza
	 * error si el usuario no existe o el alias ya está registrado.
	 *
	 * @param dniUsuario      El DNI del usuario titular de la cuenta.
	 * @param alias           El alias deseado para la cuenta.
	 ...
	 * usuario autorizado. Lanza error si el usuario no existe, el alias ya está
	 ...
	 * cuentas asociadas a un usuario. Para cada cuenta en el listado se debe con el
	 * siguiente formato: - "[Tipo]: [Alias] ([CVU])"
	 * 
	...
	/**
	 * 4) Consulta el saldo disponible de una cuenta específica. Lanza error si la
	 * cuenta no existe.
	 *
	...
	/**
	 * 5) Realiza una transferencia de dinero entre dos cuentas. Lanza error si
	...
	/**
	 * 6) Genera una nueva inversión de renta fija desde una cuenta. Lanza error si
	 * el usuario o la cuenta no existe, o si algun dato es inválido.
	 *
	 * @param dni       El DNI del usuario.
	...
	/**
	 * 6) Genera una nueva inversión en divisas extranjeras desde una cuenta. Lanza
	 * error si el usuario o la cuenta no existe, o si algun dato es inválido.
	 *
	 * @param dni       El DNI del usuario.
	 * @param cvu       El CVU de la cuenta desde donde se invierte.
	...
	/**
	 * [Nuevo] 13) Precancela una inversión activa de forma anticipada. Lanza error
	 * si algun dato es inválido, la inversión no existe o no está activa.
	 *
	 * @param dni         El DNI del usuario.
	...
	/**
	 * [Nuevo] 14) Dado un alias consultar el CVU asociado. Lanza error si el alias
	 * no está registrado.
	 *
	...
	/**
	 * 7) Obtiene el historial global de actividades del sistema. Las actividades se
	 * deben mostrar con el siguiente formato: - transferencia: ``` origen: [dni]
	 * ([cvu]) destino: [dni] ([cvu]) monto: [monto] [Aprovado/Rechazado] ``` -
	 * inversion: ``` origen: [dni] ([cvu]) desc: [tipo inversion] monto: [monto]
	 * plazo: [plazo] [Aprovado/Rechazado] ```
	 *
	...
	/**
	 * 8) Obtiene el historial de actividades asociado a una cuenta específica. Con
	 * el mismo formato que en 7) historial global
	 * 
	...
	/**
	 * 8) Obtiene el historial de actividades de un usuario a lo largo de todas sus
	 * cuentas. Con el mismo formato que en 7) historial global
	 * 
	 * Lanza error si el usuario no existe.
	 * 
	 ...
	 * sus cuentas. Lanza error si el usuario no existe.
	 *
	 * @param dniUsuario El DNI del usuario a consultar.
	...
	/**
	 * 10) Obtiene las cuentas con la mayor cantidad de actividades registradas. Se
	 * debe usar el mismo formato que el punto 3): - "[Tipo]: [Alias] ([CVU])"
	 * 
	...
	/**
	 * [Bonus Track] 15) Procesa todas las inversiones que vencen el dia de hoy y
	 * actualiza los saldos agregando los intereses generados segun el tipo de
	 * inversion. Sea por taza fija o por cotización de activos más tasa.
	 * 
